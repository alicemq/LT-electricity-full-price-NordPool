...
const loadPrices = async () => {
  try {
    ...
    console.log('Today API response:', JSON.stringify(data, null, 2)); // Detailed object view
    
    const tomorrow = moment().add(1, 'days');
    const tomorrowData = await fetchPrices(tomorrow);
    console.log('Tomorrow API response:', JSON.stringify(tomorrowData, null, 2)); // Detailed object view
    ...
    if (todayPrices.length > 0) {
      console.log('Sample price object:', JSON.stringify(todayPrices[0], null, 2));
    }
    ...
    // Log a price object sample
    if (priceData.value.length > 0) {
      console.log('Sample upcoming price object:', JSON.stringify(priceData.value[0], null, 2));
    }
  } catch (e) {
