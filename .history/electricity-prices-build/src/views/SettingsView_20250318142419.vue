  ...
  data() {
    return {
      settings: {
        ...
        cheapRange: 15,        // Default at 35%
        expensiveRange: 15     // Default at 65%
      }
    }
  },
  computed: {
    maxCheapRange() {
      return Math.min(this.settings.expensiveRange - 1, 99);
    },
    minExpensiveRange() {
      return Math.max(this.settings.cheapRange + 1, 1);
    }
  },
  watch: {
    'settings.cheapRange'(newVal) {
      // Ensure cheap range doesn't exceed expensive range
      if (newVal >= this.settings.expensiveRange) {
        this.settings.cheapRange = this.settings.expensiveRange - 1;
      }
    },
    'settings.expensiveRange'(newVal) {
      // Ensure expensive range stays above cheap range
      if (newVal <= this.settings.cheapRange) {
        this.settings.expensiveRange = this.settings.cheapRange + 1;
      }
    },
    settings: {
      handler(newSettings) {
