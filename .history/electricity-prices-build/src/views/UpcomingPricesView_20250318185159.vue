...
// Fix the upcoming prices calculation
const loadPrices = async () => {
  try {
    ...
    // Filter for upcoming prices
    const currentTime = moment().subtract(30, 'minutes'); // Include current hour
    const upcoming = allPrices.value.filter(price => 
      moment(price.timestamp).isAfter(currentTime)
    );
