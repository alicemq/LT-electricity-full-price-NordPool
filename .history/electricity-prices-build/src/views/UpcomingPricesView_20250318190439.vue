...
const loadPrices = async () => {
  try {
    ...
    // Log the entire objects for inspection
    console.log('Raw price data objects:');
    console.log('All prices array:', allPrices.value);
    console.log('Upcoming prices array:', priceData.value);
    
    // Log first object in detail if available
    if (priceData.value.length > 0) {
      console.log('First upcoming price object:', priceData.value[0]);
      
      // Also log calculated price for this object
      const firstPrice = priceData.value[0];
      console.log('Calculated price for first object:', {
        time: formatPriceHours(firstPrice.timestamp, 1),
        rawPrice: firstPrice.price,
        calculatedPrice: calculatePrice(firstPrice) + ' ct/kWh'
      });
    }
    
    // Log formatted data that users will see
    console.log('Upcoming electricity prices as shown to users:');
    priceData.value.forEach(price => {
      console.log(
        `${formatPriceHours(price.timestamp, 1)}: ${calculatePrice(price)} ct/kWh`
      );
    });
    
    console.log(`Total hours: ${allPrices.value.length}, Upcoming hours: ${priceData.value.length}`);
  } catch (e) {
