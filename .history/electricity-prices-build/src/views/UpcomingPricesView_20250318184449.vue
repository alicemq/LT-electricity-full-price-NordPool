...
const upcomingPrices = computed(() => {
  // Use moment to get current time with proper timezone
  const currentTime = moment().subtract(5, 'minutes'); // Include current hour by subtracting a few minutes
  
  // Filter prices that are in the present or future
  const filtered = priceData.value
    .filter(price => {
      const priceTime = moment(price.timestamp);
      return priceTime.isSameOrAfter(currentTime, 'hour');
    })
    .slice(0, numberOfHours.value);

  console.log('Filtered upcoming prices:', filtered);
  return filtered;
});
...
const loadPrices = async () => {
  try {
    ...
    // Add debug to verify data is being loaded
    console.log(`Loaded ${combined.length} prices in total`);
  } catch (e) {
