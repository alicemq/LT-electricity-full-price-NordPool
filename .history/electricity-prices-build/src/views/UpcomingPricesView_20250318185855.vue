...
const loadPrices = async () => {
  try {
    ...
    // Filter to show only current and future prices - fix the comparison
    const now = moment(); // Current time with timezone
    
    const upcomingPrices = combined.filter(price => {
      const priceTime = moment(price.timestamp);
      // Only show hours that are current or in the future
