...
const loadPrices = async () => {
  try {
    ...
    // Simply show all prices for now - debugging step
    priceData.value = combined;
    
    console.log(`Loaded ${allPrices.value.length} total prices`);
    
    // Log sample price data to understand structure
    if (combined.length > 0) {
      console.log('Sample price:', combined[0]);
    }
  } catch (e) {
