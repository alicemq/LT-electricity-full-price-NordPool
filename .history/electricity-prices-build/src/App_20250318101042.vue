...
const timeZones = {
  "Four zones": {
    ...
    values: {
      ...
      hours: {
        alltime: {
          mondayToFriday: ["night", "night", "night", "night", "night", "morning", "morning", "day", "day", "day", "day", "day", "day", "day", "day", "day", "day", "day", "day", "evening", "evening", "evening", "evening", "evening", "night", "night"],
...
async function getData(url, retries = 3) {
  while (retries > 0) {
    try {
      ...
      if (data.error) {
        throw new Error(data.error + (data.raw ? ` (Raw: ${data.raw})` : ''))
      }
      
      // Check data structure without throwing
      if (!data || typeof data !== 'object') {
        console.warn('Response is not an object:', data)
      }
      if (!data.data || !data.data.lt) {
        console.warn('Missing expected data structure:', data)
      }
      
      return {
        data: {
          lt: Array.isArray(data?.data?.lt) ? data.data.lt : []
        }
      }
    } catch (error) {
...
watch(date, async (newValue, oldValue) => {
  ...
  priceData.value = { lt: [] } // Initialize with empty array
  ...
  try {
    ...
    let apiUrl = `proxy.php?start=${encodeURIComponent(lastDay + 'T' + lastDayHour + ':00:00.999Z')}&end=${encodeURIComponent(currentDate + 'T' + currentDayHour + ':59:59.999Z')}`
    ...
    error.value = `Failed to load price data: ${e.message}`
  } finally {
