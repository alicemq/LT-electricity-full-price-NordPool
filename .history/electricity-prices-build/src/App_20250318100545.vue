...
const timeZones = {
  "Four zones": {
    ...
    values: {
      ...
      hours: {
        alltime: {
          mondayToFriday: ["night", "night", "night", "night", "night", "morning", "morning", "day", "day", "day", "day", "day", "day", "day", "day", "day", "day", "day", "evening", "evening", "evening", "evening", "evening", "night", "night"],
...
watch(date, async (newValue, oldValue) => {
  ...
  try {
    ...
    let apiUrl = `proxy.php?start=${lastDay}T${lastDayHour}%3A00%3A00.999Z&end=${currentDate}T${currentDayHour}%3A59%3A59.999Z`
    ...
    if (!Array.isArray(data.data.lt)) {
      console.error('Raw price data:', data.data)
      throw new Error('Invalid price data structure - lt is not an array')
    }
